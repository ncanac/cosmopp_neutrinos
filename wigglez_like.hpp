...
public:
    WiggleZLikelihood(std::string path, Cosmo& cosmo, char redshift_bin, bool initializeCosmoAtEachStep = true) : initializeCosmoAtEachStep_(initializeCosmoAtEachStep) 
    {
        ...
        // from the data.
        int ifid_discard;
        datafile.open(root_ + "gigglezfiducialmodel_matterpower_" + redshift_bin + ".dat");
        double kdum, dum1;
        datafile >> kdum >> dum1;
        int line_number = 1;
        while(kdum < kh_[0])
        ...
        datafile.open(root_ + "wigglez_nov11" + redshift_bin + "_windows.txt");
        std::string line;
        ...
        datafile.open(root_ + "wigglez_nov11" + redshift_bin + "_measurements.txt");
        ...
        datafile.open(root_ + "wigglez_nov11" + redshift_bin + "_cov.txt");
        for(int i_region = 0; i_region < num_regions_; ++i_region)
        {
            std::getline(datafile, line);
            for(int i = 0; i < num_mpk_points_full_; ++i)
            {
                ...
                while(iss >> dummy)
                    vec.push_back(dummy);
                if(i == 0 && i_region == 0 && redshift_bin == 'a')
